...
<body>
	...
	try {
		...
		String programCheckQuery = "SELECT COUNT(*) FROM MUSEUM_PROGRAM_LIST WHERE EduID = ?";
		PreparedStatement programCheckStmt = conn.prepareStatement(programCheckQuery);
		programCheckStmt.setString(1, CeduID);
		ResultSet programCheckResult = programCheckStmt.executeQuery();
		programCheckResult.next();
		int programCount = programCheckResult.getInt(1);
		programCheckStmt.close();
		programCheckResult.close();

		if (programCount == 0) {
			// The program does not exist, redirect to program_apply.jsp
			response.sendRedirect("program_apply_fail.jsp");
		} else {
			// ApplyID 설정
	...
	<div class="box">
		...
		<a href="user_view.jsp" style="color: #626A72"><h4
				style="margin-bottom: 20px;">신청내역 확인하기</h4></a>
	</div>
